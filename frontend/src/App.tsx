...
const mapFormSchema: z.ZodType<MapFormValues> = z
  .object({
    ...
    filtri: z
      .array(z.string())
      .max(placeFilters.length - 1, "Please deselect at least one POI filter."),
  ...
  .refine((data) => data.filtri.length < placeFilters.length, {
    path: ["filtri"],
    message: "Please deselect at least one POI filter.",
  });
    ...
    >
      ...
      <header className="fixed inset-x-0 top-0 z-20 navbar h-[4.5rem] border-b border-base-300/80 bg-base-100/72 px-1 backdrop-blur-md md:px-2">
        <div className="relative flex w-full items-center">
          <button
            className="btn btn-ghost text-2xl font-extrabold text-primary"
          ...
          <ul className="menu menu-horizontal absolute left-1/2 min-w-0 -translate-x-1/2 gap-2 overflow-x-auto rounded-md border border-base-300 bg-base-200/92 p-1 md:gap-3">
            {tabs.map((tab) => (
              <li key={tab.id}>
                <button
                  className={`rounded-md px-4 font-semibold hover:cursor-pointer md:px-5 ${
                    activeTab === tab.id
          ...
          <button
            className="btn btn-ghost btn-sm btn-square ml-auto rounded-md hover:cursor-pointer"
            onClick={() =>
  ...
  return (
    <section className="mx-auto w-full max-w-5xl">
      <div className="text-center">
        <h1
          className={`text-4xl font-extrabold md:text-5xl ${theme === "dark" ? "text-white" : "text-slate-900"}`}
        >
          Eventour Open Data
        </h1>
        <p
          className={`mt-4 md:text-xl ${theme === "dark" ? "text-white" : "text-slate-800"}`}
        >
      ...
      <div className="mx-auto grid h-full w-full max-w-[1160px] grid-rows-[auto_1fr_auto] gap-3">
        <section className="rounded-lg border border-base-300/70 bg-base-100/90 px-5 py-4 shadow-sm">
          <div className="grid items-center gap-4 md:grid-cols-[1.2fr_1fr]">
            <div>
              <p className="text-xs uppercase tracking-wide text-base-content/65">
                Urban Intelligence
              </p>
              <h3 className="mt-1 text-xl font-bold">
                Explore Eventour Graph Insights
              </h3>
              <p className="mt-1 text-sm text-base-content/75">
                Discover connected city entities for itineraries, cultural
                discovery, and semantic exploration.
              </p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-xs md:grid-cols-4">
          <div>
            ...
            <p className="mt-1 text-base-content/70">
              Semantic urban graph platform for Milan.
            </p>
            <div className="mt-2 flex items-center gap-2">
              <span className="text-base-content/75">Coverage</span>
          ...
          <div>
            ...
            <div className="mt-2 grid gap-1.5">
              <div className="flex items-center gap-2">
                <span className="text-base-content/75">Map</span>
                <span className="badge badge-primary badge-sm rounded-md">
                  Explorer
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-base-content/75">SPARQL</span>
          ...
          <div>
            ...
            <div className="mt-2 grid gap-1.5">
              <div className="flex items-center gap-2">
                <span className="text-base-content/75">Graph</span>
                <span className="badge badge-accent badge-sm rounded-md">
                  Knowledge Base
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-base-content/75">Sources</span>
          ...
          <div>
            ...
            <div className="mt-2 grid gap-1.5">
              <div className="flex items-center gap-2">
                <span className="text-base-content/75">Team</span>
                <span className="badge badge-warning badge-sm rounded-md">
                  Research & Product
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-base-content/75">Location</span>
                <span className="badge badge-neutral badge-sm rounded-md">
                  Milan, Italy
                </span>
              </div>
            </div>
...
function MapPage({ theme }: { theme: Theme }) {
  ...
  const [selectedItineraryPath, setSelectedItineraryPath] = useState<
    string | null
  >(null);
  ...
  const selectedItinerary = useMemo(
    () =>
      itineraries.find(
        (itinerary) => itinerary.percorso === selectedItineraryPath,
      ) ?? null,
  ...
  const selectedPoint = useMemo(() => {
    ...
    const route = itineraries.find(
      (itinerary) => itinerary.percorso === selectedRoutePoint.route,
    );
    if (!route) {
      return null;
    }
    ...
    const fetchPois = async () => {
      ...
      try {
        for (const candidate of candidates) {
          try {
            const response = await fetch(`${candidate}/pois`, {
            ...
            const options = Array.isArray(data.items)
              ? (data.items as PoiOption[])
              : [];
            ...
            if (!getValues("nodo_arrivo")) {
              setValue(
                "nodo_arrivo",
                options[1]?.poi_id || options[0]?.poi_id || "",
              );
            }
        ...
        setRequestMessage(
          error instanceof Error
            ? error.message
            : "Unable to load the POI list from the backend.",
        );
      }
    };
  ...
  const onSubmit = async (values: MapFormValues) => {
    ...
    if (values.nodo_partenza === values.nodo_arrivo) {
      setRequestState("error");
      setRequestMessage("Start and destination must be different.");
      return;
    }

    if (values.filtri.length === placeFilters.length) {
      setRequestState("error");
      setRequestMessage("Please deselect at least one POI filter.");
      return;
    }
    ...
    try {
      ...
      const extractedItineraries = Array.isArray(result.itinerari)
        ? result.itinerari
        : [];
      ...
      if (extractedItineraries.length > 0) {
        ...
        setRequestMessage(
          result.llm_response ||
            "No routes found for the selected constraints.",
        );
      }
    } catch (error) {
      setRequestState("error");
      ...
      <section className="h-full">
        ...
        >
          <TileLayer
            ...
            url={mapTile.url}
            className={theme === "dark" ? "map-dark-base" : undefined}
          />
          {theme === "dark" && mapTile.labelsUrl ? (
            <TileLayer
              attribution={mapTile.attribution}
              url={mapTile.labelsUrl}
                ...
                eventHandlers={{
                  click: () => {
                    if (selectedItinerary) {
                      setSelectedRoutePoint({
                        route: selectedItinerary.percorso,
                        index,
                      });
                    }
                  },
...
function SparqlPage() {
  ...
  const runQuery = async () => {
    ...
    try {
      ...
      setRequestState("success");
      setRequestMessage(
        result.message || `Query completed. ${nextRows.length} rows.`,
      );
    } catch (error) {
      setRequestState("error");
      setRows([]);
        ...
        <div className="card-body gap-3">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                className="btn btn-ghost btn-sm rounded-md hover:cursor-pointer"
                onClick={handleReset}
              >
                <RotateCcw size={16} />
                Reset
              ...
              <button
                type="submit"
                form="sparql-form"
                className="btn btn-primary btn-sm rounded-md hover:cursor-pointer"
              ...
              {sparqlExamples.map((example) => {
                ...
                return (
                  <button
                    key={example.label}
                    type="button"
                    className={`card rounded-md border bg-gradient-to-br text-left transition hover:cursor-pointer ${
                      isActive
                  ...
                  >
                    <div className="card-body gap-2 px-3 py-3">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="badge badge-outline badge-xs rounded-md">
                          Example
                        </span>
                      </div>
                      <p className="text-xs text-base-content/70">
      ...
      {queryExecuted && queryExecuted !== query.trim() ? (
        <div className="alert alert-info mt-3 rounded-md">
          <span className="text-xs break-words">
            Query normalized automatically for GraphDB: GRAPH http://... becomes
            GRAPH {"<"}http://...{">"}
          </span>
        </div>
      ) : null}
      ...
      <article className="card mt-6 rounded-md border border-base-300 bg-base-100/92 shadow-sm">
        <div className="card-body">
          ...
          <p className="text-base-content/80">
            Eventour is a semantic urban knowledge graph for Milan that connects
            cultural places, public services, transport, green assets, and
            neighborhood context into one integrated city resource. It helps
            applications support smarter event discovery, cultural itineraries,
            and context-aware urban exploration.
          </p>
        </div>
      </article>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-3">
        <article className="card rounded-md border border-base-300 bg-base-100/90 shadow-sm">
          <div className="card-body">
            ...
            <p>
              Eventour is an application used to improve crowd management after
              large events by guiding attendees through personalized walking
              itineraries before they head to transit stations. It uses
              LLM-generated routes, nearby points of interest, and gamified quiz
              content to make dispersal more engaging while helping reduce
              post-event congestion around metro and public transport stops.
            </p>
          </div>
        </article>
        <article className="card rounded-md border border-base-300 bg-base-100/90 shadow-sm">
          <div className="card-body">
            ...
            <p>
              The platform is powered by linked data about the city of Milan,
              including geospatial datasets represented as a knowledge graph.
              This model connects places, services, transport context, and
              event-relevant entities into one interoperable semantic layer.
            </p>
          </div>
        </article>
        <article className="card rounded-md border border-base-300 bg-base-100/90 shadow-sm">
          <div className="card-body">
            ...
            <p>
              The map uses these linked geospatial data to define different
              interest-based routes. It is designed for event contexts, helping
              distribute crowds through user-adapted paths and supporting
              smoother flows toward stations and mobility hubs.
            </p>
          </div>
        </article>
      </div>
